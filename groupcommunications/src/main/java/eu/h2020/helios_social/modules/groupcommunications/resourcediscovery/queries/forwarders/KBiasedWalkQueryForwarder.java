...
public class KBiasedWalkQueryForwarder extends QueryForwarder {
    private static Logger LOG = getLogger(KBiasedWalkQueryForwarder.class.getName());
    ...
    public KBiasedWalkQueryForwarder(ContactManager contactManager, HashMap<PeerId,Double> distribution, int K) {
    ...
    public List<PeerId> forward(PeerId peerId, Query query) throws DbException {
        ...
        while (i < K && !neighbors.isEmpty()) {
			...
			probabilities.remove(nextForward);
            probabilities = normalize(probabilities);
            i++;
        }
        return nextForwards;
