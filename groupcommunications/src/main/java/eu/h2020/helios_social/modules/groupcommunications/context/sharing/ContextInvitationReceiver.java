...
public class ContextInvitationReceiver implements HeliosMessagingReceiver {
    private final Logger LOG = Logger.getLogger(ContextInvitationReceiver.class.getName());
    ...
    public void receiveMessage(
        ...
        ContactId contactId = new ContactId(heliosNetworkAddress.getNetworkId());
        if (protocolId.equals(CONTEXT_INVITE_PROTOCOL)) {
            ...
            try {
                if (contextManager.contextExists(contextInvitation.getContextId())) {
                    eventBus.broadcast(new ContextInvitationAutoResponseEvent(
                            contextInvitation));
                } else {
